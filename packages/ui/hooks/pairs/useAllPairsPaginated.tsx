...
import { fetchAllPairs } from 'queries';
import { useState } from 'react';
import type { PairData } from 'types';

export const useAllPairsPaginated = (network: string, pageSize: number) => {
  ...
  const fetchWithPage = async (page: number) => {
    try {
      const pairs: PairData = await fetchAllPairs(apiUrl, pageSize, page);
      return pairs;
    } catch (error) {
