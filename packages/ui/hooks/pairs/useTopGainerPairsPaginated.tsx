...
import { fetchTopGainerPairs } from 'queries';
import { useState } from 'react';
import type { PairData } from 'types';

export const useTopGainerPairsPaginated = (network: string, pageSize: number) => {
  ...
  const fetchWithPage = async (page: number) => {
    try {
      const pairs: PairData = await fetchTopGainerPairs(apiUrl, pageSize, page);
      return pairs;
    } catch (error) {
